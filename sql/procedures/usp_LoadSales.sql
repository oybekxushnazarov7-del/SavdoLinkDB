...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @FutureDates INT;

        SELECT @FutureDates = COUNT(*)
        FROM stg.RawSales rs
        WHERE rs.LoadId = @LoadId
          AND COALESCE(
        ...
        IF @FutureDates > 0
            ...
            VALUES (
                @LoadId, 0,
                CONCAT(N'Kelajakdagi sana — core ga yuklanmadi: ', @FutureDates),
                N'core.usp_LoadSales'
            );
        ...
        USING (
            SELECT ReceiptNo, StoreId, EmployeeId, SaleDateTime, PaymentType
            FROM (
                SELECT
                    ReceiptNo, StoreId, EmployeeId, SaleDateTime, PaymentType,
                    ROW_NUMBER() OVER (
                        PARTITION BY ReceiptNo, StoreId
                        ORDER BY SaleDateTime
                    ) AS rn
                FROM #TempSales
                WHERE SaleDateTime IS NOT NULL
                  AND SaleDateTime <= SYSDATETIME()
